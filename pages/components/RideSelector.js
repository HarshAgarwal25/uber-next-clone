...
const RideSelector = ({ pickupCoordinates, dropoffCoordinates }) => {
  ...
  useEffect(() => {
    rideDuration = fetch(
      `https://api.mapbox.com/directions/v5/mapbox/driving/${pickupCoordinates[0]},${pickupCoordinates[1]};${dropoffCoordinates[0]},${dropoffCoordinates[1]}?access_token=pk.eyJ1IjoiaGFyc2hhZ2Fyd2FsMjUiLCJhIjoiY2t3M2Z4aHNhMGFsdDJwcHhxNDg5bjBqMiJ9.fHcPQ6954PZf1Aw5CaB-vQ`
    )
      .then((res) => res.json())
      .then((data) => {
      ...
      <CarList>
        {carList.map((car, index) => (
          <Car key={index}>
            ...
            <Price>{'₹'+(rideDuration*car.multiplier*12).toFixed(2)}</Price>
          </Car>
        ))}
      </CarList>
    </Wrapper>
